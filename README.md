...
домен прокси, существующий ключ (его можно оставить пустым), HTTPS-сайт для
маскировки и подтверждение установки. Для сайта маскировки можно ввести просто
домен, например `example.com`; соединение с ним всё равно выполняется по TLS.
Email не требуется. Домен прокси должен указывать на сервер; TCP-порты 80 и 443
должны быть доступны для Caddy.
